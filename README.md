...
2. After the buyer linked the items account. We will extract income and asset reports from Items sets. ~~Note I couldn't get a working Asset Report because we need to provide a webhook for async creation of asset report. This is can only be done if our server was live with exposed ip address~~. I used ngrok to expose my local ip to the web. Any traffic that hit ngrok endpoint will be forwarded to my local ip and port. I then provided the webhook to Plaid so that when Plaid send data back, it will be pushed to client using Socket.io.
